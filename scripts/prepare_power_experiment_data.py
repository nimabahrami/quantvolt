...
def main() -> None:
    prices = pl.read_parquet(PROCESSED / "smard_de_lu_day_ahead_native.parquet").select(
    ...
    physical = pl.read_parquet(PROCESSED / "smard_de_power_fundamentals_quarter_hour.parquet").sort(
        "interval_start_utc"
    )
