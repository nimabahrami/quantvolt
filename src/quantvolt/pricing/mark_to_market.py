...
class MtMPosition:
    """An open position to be marked to market.
...
class MtMPositionResult:
    """Per-position mark, P&L pair, and how the mark was sourced."""
...
def _resolve_mark(
    ...
    2. Otherwise a caller-supplied ``forward_curve`` *for the same commodity* whose
       node covers the period → ``"estimated"``. A curve for a different
    ...
    3. Otherwise raise :class:`NoPricingDataError` naming the commodity and delivery
       period — the position is never silently omitted.
    """
...
def mark_to_market(
    ...
    """Mark each position to market and compute daily and cumulative P&L.
